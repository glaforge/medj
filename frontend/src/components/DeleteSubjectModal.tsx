...
interface DeleteSubjectModalProps {
  ...
  coursesCount?: number;
  onConfirmDelete: (subjectId: string) => Promise<void> | void;
}
...
export const DeleteSubjectModal: React.FC<DeleteSubjectModalProps> = ({
  ...
  const subjectColor = subject.color || '#0284c7';

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsDeleting(true);
    try {
      await onConfirmDelete(subject.id);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="glass-panel border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-scaleUp bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white truncate">
                Supprimer une UE / Matière
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 truncate">
                Confirmation de suppression de l'Unité d'Enseignement
              </p>
            </div>
          </div>
        ...
        {/* Content Body */}
        <form id="delete-subject-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5 text-xs">
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              <span
                className="px-2.5 py-1 rounded-lg text-xs font-extrabold uppercase tracking-wider shadow-2xs"
                style={{
                  backgroundColor: subjectColor,
                  color: getContrastTextColor(subjectColor)
                }}
              >
                {subject.code}
              </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-md bg-sky-100 dark:bg-sky-950/60 border border-sky-300 dark:border-sky-800/30 text-[10px] font-bold text-sky-700 dark:text-sky-400">
                  {coursesCount} cours rattaché(s)
                </span>
              </div>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/60 flex items-start gap-3">
            ...
            <div className="space-y-1 text-slate-700 dark:text-slate-300">
              <div className="font-bold text-rose-700 dark:text-rose-300">
                {coursesCount > 0
                  ? `Attention : ${coursesCount} cours seront supprimés`
                  : 'Cette action est irréversible'}
              </div>
              <p className="text-[11px] leading-relaxed">
                {coursesCount > 0
                  ? `La suppression de l'UE « ${subject.code} » entraînera la suppression définitive de l'ensemble de ses ${coursesCount} cours ainsi que toutes leurs séances de révision planifiées.`
                  : `Cette UE sera définitivement retirée de votre cursus. Vos plannings et cours restants ne seront pas impactés.`}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/50 flex items-center justify-end gap-3 shrink-0">
          ...
          >
            ...
            <span>{isDeleting ? 'Suppression...' : 'Supprimer définitivement cette UE'}</span>
          </button>
        </div>
